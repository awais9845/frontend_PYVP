...
import { Menu, X, Landmark, User as UserIcon, LogOut, ShieldAlert, CheckCircle, Moon, Sun } from "lucide-react";
...
export default function Navbar() {
  ...
  const navLinks = [
    ...
    { name: "Join Parliament", path: "/register" },
    ...
    >
      <div className="max-w-7xl mx-auto px-4 md:px-8 flex items-center justify-between">
        ...
        <div className="hidden lg:flex items-center gap-3">
          ...
          >
            {isDark ? <Sun className="h-5 w-5 text-gold-500" /> : <Moon className="h-5 w-5 text-slate-600" />}
          </button>
          ...
          {user ? (
            <div className="flex items-center gap-2 border-l border-slate-200 dark:border-slate-700 pl-3">
              ...
              >
                {user.profilePic ? (
                  <img src={user.profilePic} alt="pic" className="h-7 w-7 rounded-full object-cover border border-emerald-600" />
                ) : (
            ...
            {user ? (
              <div className="space-y-2.5">
                <div className="flex items-center gap-3 px-4">
                  {user.profilePic ? (
                    <img src={user.profilePic} alt="pic" className="h-9 w-9 rounded-full object-cover border border-emerald-600" />
                  ) : (
                  ...
                  <div>
                    <h4 className="font-bold text-sm text-slate-800 dark:text-slate-100">{user.fullName}</h4>
