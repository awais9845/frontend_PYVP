...
export default function ChairmanDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<
    "overview" | "cabinet" | "announcement" | "applications"
  >("overview");
  ...
  const [editingCabinetMember, setEditingCabinetMember] = useState<any | null>(
    null,
  );
  ...
  const [viewingCabinetMember, setViewingCabinetMember] = useState<any | null>(
    null,
  );

  // Revocation Modal States
  const [terminatingCabinetMember, setTerminatingCabinetMember] = useState<
    any | null
  >(null);
  ...
  const handleAssignCabinet = async (e: React.FormEvent) => {
    ...
    if (!selectedMemberObj || !selectedRole) {
      triggerToast(
        ...
        "Please select a candidate member and role.",
        "error",
      );
      return;
    }
    ...
    try {
      ...
      if (res.success) {
        triggerToast(
          "Role Appointed",
          res.message || "Cabinet position successfully appointed.",
          "success",
        );
      ...
      triggerToast(
        "Appointment Failed",
        err.response?.data?.message || "Could not appoint role.",
        "error",
      );
    } finally {
      setAssigning(false);
  ...
  const handleUpdateCabinet = async (e: React.FormEvent) => {
    ...
    setUpdating(true);
    try {
      const res = await cabinetService.updateExecutiveRole(
        editingCabinetMember._id,
        {
      ...
      if (res.success) {
        triggerToast(
          "Assignment Updated",
          res.message || "Executive role updated successfully.",
          "success",
        );
      ...
      triggerToast(
        "Update Failed",
        err.response?.data?.message || "Could not save portfolio.",
        "error",
      );
    } finally {
      setUpdating(false);
  ...
  const handleRemoveCabinet = async () => {
    ...
    try {
      const res = await cabinetService.removeExecutiveRole(
        terminatingCabinetMember._id,
      );
      if (res.success) {
        triggerToast(
          "Cabinet Revoked",
          res.message || "Cabinet position successfully removed.",
          "success",
        );
      ...
      triggerToast(
        "Revocation Failed",
        err.response?.data?.message || "Could not remove cabinet role.",
        "error",
      );
    } finally {
      setDeleting(false);
  ...
  const handlePublishAnnouncement = async (e: React.FormEvent) => {
    ...
    if (!announcementTitle.trim() || !announcementDesc.trim()) {
      triggerToast(
        "Validation Alert",
        "Title and description details cannot be blank.",
        "error",
      );
      return;
    }
    ...
    try {
      ...
      if (res.success) {
        triggerToast(
          "Announcement Active",
          "Homepage public announcement successfully published and refreshed.",
          "success",
        );
        loadDashboardData();
      }
      ...
      triggerToast(
        "Publication Failed",
        err.response?.data?.message || "Could not publish announcement.",
        "error",
      );
    } finally {
      setPublishing(false);
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-start md:items-center gap-6 relative z-10">
          <div className="space-y-2">
            ...
            <p className="text-xs sm:text-sm text-slate-300 max-w-2xl font-light">
              Manage executive roles, configure homepage announcements, inspect
              dossier registries, and oversee the Youth Parliament cabinet
              structure.
            </p>
          </div>
                ...
                {currentAnnouncement ? (
                  ...
                  <div className="text-center py-10 border-2 border-dashed border-slate-200 dark:border-slate-850 rounded-xl space-y-2 text-slate-400">
                    ...
                    <p className="text-xs font-bold">
                      No Active Homepage Announcement
                    </p>
                    <p className="text-[10px] max-w-xs mx-auto">
                      Publish an official notice from the management tab to
                      replace the default banner text on the index homepage.
                    </p>
                  </div>
                )}
              </div>
              ...
              {loadingCabinet ? (
                ...
                <div className="bg-white dark:bg-slate-900 rounded-2xl border-2 border-dashed border-slate-250 p-16 text-center space-y-4 shadow-xs">
                  ...
                  <p className="text-xs text-slate-450 max-w-sm mx-auto">
                    No leadership portfolios are active. Assign cabinet roles to
                    approved general members on the right.
                  </p>
                </div>
              ) : (
                      ...
                      <tbody className="divide-y divide-slate-50 dark:divide-slate-800/50">
                        {cabinetMembers
                          ...
                          .map((m) => {
                            const avatarRaw =
                              m.profileImage ||
                              m.user?.profileImage?.secure_url ||
                              null;
                            const avatar = avatarRaw
                              ? getOptimizedCloudinaryUrl(avatarRaw, 100, 100)
                              : null;
                            const initials =
                              m.fullName
                                ?.split(" ")
            ...
            <div className="xl:col-span-4 space-y-6">
              ...
              {selectedMemberObj ? (
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex items-center gap-3">
                      <div className="h-12 w-12 rounded-full overflow-hidden bg-slate-950 flex items-center justify-center border border-emerald-600 shrink-0">
                        {selectedMemberObj.profileImage ||
                        selectedMemberObj.user?.profileImage?.secure_url ? (
                          <img
                            src={getOptimizedCloudinaryUrl(
                              selectedMemberObj.profileImage ||
                                selectedMemberObj.user?.profileImage
                                  ?.secure_url,
                              120,
                              120,
                            )}
                          ...
                          <span className="font-bold text-lg text-white">
                            {selectedMemberObj.fullName?.charAt(0)}
                          </span>
                        )}
                      </div>
                      <div>
                        <h4 className="font-heading font-bold text-slate-900 dark:text-white leading-tight">
                          {selectedMemberObj.fullName}
                        </h4>
                        <span className="text-[10px] text-slate-400 font-mono block">
                          {selectedMemberObj.membershipId}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="border-t border-slate-100 dark:border-slate-800 pt-3 space-y-2 text-xs">
                      <div className="flex justify-between">
                        ...
                        <strong className="text-slate-700 dark:text-slate-350 truncate max-w-[160px]">
                          {selectedMemberObj.email}
                        </strong>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-slate-400 flex items-center gap-1">
                          <MapPin className="h-3 w-3" /> Seat:
                        </span>
                        <strong className="text-slate-700 dark:text-slate-350 truncate">
                          {selectedMemberObj.district
                            ? `${selectedMemberObj.district}, `
                            : ""}
                          {selectedMemberObj.province}
                        </strong>
                      </div>
                    </div>
                  </div>
                ...
                <div className="border border-dashed border-slate-200 dark:border-slate-850 rounded-2xl p-8 text-center text-slate-400 text-xs font-medium">
                  Select a member via candidate search to manage executive
              ...
              >
                <div className="space-y-1.5">
                  ...
                  <p className="text-[10px] text-slate-400">
                    Keep it short, clear, and highly professional.
                  </p>
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  ></textarea>
                  <p className="text-[10px] text-slate-400">
                    Provide official details, contact coordinate guidelines, or
                    instruction updates.
                  </p>
                </div>
                ...
                <div className="border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden shadow-xs bg-slate-50 dark:bg-slate-950 p-4">
                  ...
                  <div className="bg-emerald-900 text-white py-2 px-3 border-b border-emerald-800 text-[10px] font-semibold flex items-center gap-2 select-none rounded-t-lg">
                    ...
                    <span className="truncate">
                      📢{" "}
                      {announcementTitle ||
                        "Official PYVP public notice is actively broadcasted here..."}
                    </span>
                  </div>
                  ...
                  <div className="mt-4 p-5 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 space-y-3 shadow-xs relative">
                    ...
                    <p className="text-[11px] text-slate-500 dark:text-slate-400 leading-relaxed font-light whitespace-pre-line">
                      {announcementDesc ||
                        "Your official parliamentary notices, administrative announcements, intake guidelines, or event agendas will be rendered here dynamically for all public site visitors."}
                    </p>
            ...
            <div className="pb-4 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center">
              <div>
                ...
                <p className="text-[11px] text-slate-450">
                  Showing recent membership applications received at the
                  registry.
                </p>
              </div>
              ...
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <p className="text-xs text-slate-405">
                    Editing assignment for {editingCabinetMember.fullName}
                  </p>
                </div>
                <button
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 max-w-md w-full rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="p-6 md:p-8 space-y-6">
              <div className="flex gap-3 items-start text-red-650">
                ...
                <div>
                  <h4 className="font-heading font-extrabold text-lg text-slate-900 dark:text-white">
                    Revoke Portfolio Appointment
                  </h4>
                  <p className="text-xs text-slate-400">
                    Removing {terminatingCabinetMember.fullName} from{" "}
                    {terminatingCabinetMember.executiveRole}
                  </p>
                </div>
              </div>
              ...
              <div className="bg-red-50 dark:bg-red-950/20 border border-red-200 dark:border-red-900/35 p-3 rounded-lg flex items-start gap-2">
                ...
                <label
                  ...
                  className="text-[11px] text-red-750 dark:text-red-400 font-bold leading-snug cursor-pointer select-none"
                >
                  I confirm that I am removing this member from the Executive
                  Cabinet.
                </label>
              </div>
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 max-w-lg w-full rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="p-6 md:p-8 space-y-6">
              <div className="flex justify-between items-start">
                <div>
                  <h4 className="font-heading font-extrabold text-lg text-slate-900 dark:text-white">
                    Cabinet Portfolio Details
                  </h4>
                  <p className="text-xs text-slate-400">
                    Certified Official Profile
                  </p>
                </div>
                <button
              ...
              <div className="flex flex-col sm:flex-row items-center gap-6 pb-4">
                <div className="h-20 w-20 rounded-full border-4 border-emerald-600 overflow-hidden bg-slate-900 flex items-center justify-center shadow-lg shrink-0">
                  {viewingCabinetMember.profileImage ||
                  viewingCabinetMember.user?.profileImage?.secure_url ? (
                    <img
                      src={getOptimizedCloudinaryUrl(
                        viewingCabinetMember.profileImage ||
                          viewingCabinetMember.user?.profileImage?.secure_url,
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs border-t border-slate-100 dark:border-slate-800 pt-4">
                ...
                <div className="space-y-1">
                  ...
                  <strong className="text-slate-800 dark:text-slate-200 block">
                    {viewingCabinetMember.district
                      ? `${viewingCabinetMember.district}, `
                      : ""}
                    {viewingCabinetMember.province}
                  </strong>
                </div>
                <div className="space-y-1">
                  <span className="text-slate-400 font-medium block">
                  ...
                  <strong className="text-slate-800 dark:text-slate-200 block">
                    {viewingCabinetMember.appointmentDate
                      ? new Date(
                          viewingCabinetMember.appointmentDate,
                        ).toLocaleDateString()
                      : "N/A"}
                ...
                <div className="space-y-1">
                  ...
                  <strong className="text-slate-800 dark:text-slate-200 block">
                    {viewingCabinetMember.appointedBy?.fullName ||
                ...
                <div className="space-y-1">
                  ...
                  <strong className="text-slate-800 dark:text-slate-200 block">
                    {viewingCabinetMember.membershipType || "General"}
                  </strong>
                </div>
              </div>
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 max-w-sm w-full rounded-2xl shadow-2xl p-6 space-y-6 animate-in zoom-in-95 duration-200">
            <div className="flex gap-3 items-start text-red-650">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400">
                  This action will remove the homepage banner notice.
                </p>
              </div>
            </div>

            <p className="text-xs text-slate-500 leading-relaxed">
              Are you sure you want to delete the active announcement? The home
              page will return to displaying the default welcome message ticker.
            </p>
