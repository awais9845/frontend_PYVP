  ...
  const registerUser = async (formData: any): Promise<boolean> => {
    try {
      ...
      if (registerThunk.fulfilled.match(result)) {
        ...
        // If backend already created application (or user has application reference), succeed immediately
        if ((newUser as any)?.application) {
          triggerToast(
            "Application Submitted",
            "Your account has been created and your application is now under review by the Chairman.",
            "success"
          );
          return true;
        }

        // Secondary fallback: Submit the application record if not automatically generated by backend
        const appPayload = {
          user:          newUser._id,
          fullName:      formData.fullName,
          email:         formData.email,
          cnic:          formData.cnic || `CNIC-${Date.now()}`,
          education:     formData.education || "N/A",
          address:       `${formData.constituency || ""}, ${formData.province || ""}`.trim() || "N/A",
          receipt: {
            secure_url: formData.paymentReceipt || "https://res.cloudinary.com/rgvvrye1/image/upload/v1784959506/pyvp/receipts/recj9aep6gfa5skfh5u1.png",
            public_id:  "receipt"
          },
        ...
        try {
          ...
          // User was created successfully even if secondary app post errored
          triggerToast(
            "Account Created",
