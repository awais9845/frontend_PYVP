  ...
  events: [
    {
      ...
      description:
        "A comprehensive 3-day simulated legislative workshop in Islamabad. Selected members from all provinces will draft, debate, and pass mock youth-focused resolutions regarding economic reforms, educational access, and IT infrastructure development.",
      date: "2026-08-15",
      location:
        "Prime Minister Youth Secretariat / National Assembly Hall, Islamabad",
      status: "upcoming",
    },
    {
      id: "evt-02",
      ...
      description:
        "A prestigious assembly connecting Pakistan's youth leaders with veteran parliamentarians, federal ministers, and policy experts. Key topics include digital governance, public-private partnerships, and green initiatives.",
      date: "2026-09-22",
    ...
    },
    {
      ...
      description:
        "The official swearing-in ceremony and constitutional drafting workshop for the newly selected executive cabinet of Pakistan Youth Vision Parliament.",
      date: "2025-10-15",
  ...
  news: [
    {
      ...
      summary:
        "PYVP introduces an all-new digital system to steamline youth registration, application review, digital ID card issuance, and verification.",
      content:
        "Islamabad — In a historic step towards modernizing digital youth governance, the Chairman of the Pakistan Youth Vision Parliament (PYVP) has officially inaugurated the customized digital web portal. The system features streamlined application forms, dynamic verification portals with digital QR codes, custom member dashboards, and secure administrator reviews. 'This portal bridges the gap between state governance and youth ambition, making registration transparent and swift,' noted Hafiz Huzaifa Shah during the press conference.",
      date: "2026-06-25",
    ...
    },
    {
      ...
      summary:
        "Young Pakistani citizens aged 18 to 32 are invited to apply for the upcoming simulated parliamentary term. Registration closes soon.",
      content:
        "National — The Pakistan Youth Vision Parliament has opened public registrations for the 2026 legislative sessions. Eligible candidates from Balochistan, Khyber Pakhtunkhwa, Punjab, Sindh, Gilgit-Baltistan, AJK, and Islamabad can submit their credentials via the official portal. Applicants will submit an official document (CNIC/B-Form) along with their nomination essay and EasyPaisa registration fee receipt. Selected applicants will receive official digital certificates and ID cards enabling full floor access.",
      date: "2026-06-20",
  ...
  notifications: [
    {
      ...
      message:
        "Public applications are now open for the 2026 Youth Assembly Session. Please complete your registration and upload the required EasyPaisa payment receipt to proceed.",
...
if (process.env.NODE_ENV === "production" || process.env.USE_MOCK === "true") {
    ...
    try {
      ...
      if (!fullName || !email || !cnic || !password) {
        return res.status(400).json({
          error: "Missing required fields (fullName, email, cnic, password)",
        });
      }
      ...
      const exists = db.users.find(
        (u) => u.email.toLowerCase() === email.toLowerCase() || u.cnic === cnic,
      );
      if (exists) {
        return res.status(400).json({
          error: "Email or CNIC already registered under another application",
        });
      }
      ...
      const newNotif: Notification = {
        ...
        message:
          "Your application for the Pakistan Youth Vision Parliament has been received. Our administration will verify your EasyPaisa receipt and academic credentials within 48-72 hours.",
      ...
      res.status(500).json({
        error: e.message || "Internal server error during registration",
      });
    }
  });
  ...
  app.post("/api/auth/login", (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: e.message || "Internal server error during login" });
    }
  });
  ...
  app.put("/api/auth/profile", (req, res) => {
    try {
      ...
      if (!userId) {
        return res
          .status(400)
          .json({ error: "User ID is required to update profile" });
      }
      ...
      res.status(500).json({
        error: e.message || "Internal server error during profile update",
      });
    }
  });
  ...
  app.get("/api/public/stats", (req, res) => {
    try {
      ...
      const approved = db.users.filter(
  ...
  app.post("/api/admin/applications/:id/review", (req, res) => {
    try {
      ...
      if (!status || !["approved", "rejected"].includes(status)) {
        return res
          .status(400)
          .json({ error: "Review status must be 'approved' or 'rejected'" });
      }
      ...
      if (status === "approved") {
        ...
        const seq = String(
          db.users.filter((u) => u.status === "approved" && u.membershipId)
            .length + 1,
        ).padStart(4, "0");
  ...
  app.put("/api/admin/members/:id/role", (req, res) => {
    try {
      ...
      if (role === "executive") {
        db.users[index].executivePosition =
          executivePosition || "Cabinet Member";
      } else {
    ...
    try {
      ...
      const user = db.users.find(
        (u) =>
          ...
          ((u.membershipId && u.membershipId.toUpperCase() === term) ||
            (u.certificateNumber &&
              u.certificateNumber.toUpperCase() === term) ||
            (u.cnic &&
      ...
      if (!user) {
        return res.status(404).json({
          verified: false,
          error: "No verified record matches this credential",
        });
      }
  ...
  app.post("/api/admin/notifications", (req, res) => {
    try {
      ...
      if (!title || !message) {
        return res
          .status(400)
          .json({ error: "Notification must contain title and message" });
      }
...
const startServer = async () => {
  ...
  app.listen(PORT, "0.0.0.0", () => {
    console.log(
      `PYVP Full Stack Server running securely on http://localhost:${PORT}`,
    );
  });
};
